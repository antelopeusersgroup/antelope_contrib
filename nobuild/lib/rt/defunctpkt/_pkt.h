 ...
 *
 *====================================================================*/
#ifndef _pkt_h_included
#define _pkt_h_included
#include <sys/types.h>
#include <stdio.h>
#include "db.h"
#include "coords.h"
...
typedef struct ethrhdr {
	...
	ushort_t aux_data[7];     /* quick look at 8 data channels */ 
	ushort_t test_amplitude;  /* test bus amplitude (unused in 24 bit syste m)*/ 
	ushort_t test_rate;       /* test bus signal period (unused in 24 bit s ystem)*/ 
	ushort_t test_width;      /* test bus pulse width (unused in 24 bit sys tem)*/
													        
	ushort_t byte_count;      /* packet length */
	struct { 
	    uchar_t relay;        /* channel relay code (unused in 24 bit syste m)*/ 
	    uchar_t gain;         /* channel gain code  (unused in 24 bit syste m)*/ 
	} set[6]; 
	ushort_t comm_err_count;  /* command channel comm error count */ 
	uchar_t  send_backlog;    /* send buffer backlog count */ 
	uchar_t  smprat;          /* Sample rate 100 or 40 or 5 (status)*/ 
	uchar_t  flag;            /* time_rcv*2 + reset */ 
	uchar_t  test_attenuation;        /* test bus signal attenuation (unuse d in 24 bit system)*/ 
	ushort_t sync_latch;      /* sync clock latch value */ 
	ushort_t battery;         /* battery voltage */ 
	uint   tag_seconds;       /* time tag in seconds since year begin */ 
	uchar_t   station;        /* Block station */ 
	uchar_t   rexmit;         /* # re xmit */ 
	int   xmt_seconds;        /* xmit time tag in seconds */
} PACKHDR;
...
struct sdESP {
        ushort_t sync;            /* Software sync pattern 0xABEF =  Status */
        ushort_t checksum;        /* block checksum (16 bit XOR of packet)*/
        ushort_t unitid;          /* unit id number */
        ushort_t buf_id;          /* buffer id for rexmit*/
        ushort_t spare[2];
        ...
        ushort_t byte_count;      /* packet length */
        ushort_t spare2[5];
        ushort_t PPSjerk;        /* Remote 1PPS jerk count */
        ushort_t TimSeqErr;       /* Remote radio clock sequence error count*/
        ushort_t smprat;          /* 5 */
        uchar_t  clock_stat;      /* Remote clock status code 
	                             (1=NO1HZ, 2=NOCLO CK) */        
        uchar_t  haz_reset;       /* remote hazard or 0xAB = reset */
        ushort_t clk_unlock;      /* Count of unlock transitions */
        ulong  clk_unlock_cnt;    /* Seconds since last lock */
        ushort_t crc_cnt[16];     /* DAS crc error count */
        ushort_t chksum_cnt[16];  /* DAS checksum error count */
        ushort_t reset_cnt[16];   /* DAS reset count */
        ushort_t no_recv[16];     /* DAS command receive timeout count */        
        ushort_t tim_err[16];     /* DAS second compare error count */
        ushort_t msec_err[16];    /* DAS msec error count */
        ushort_t rtx_snd_cnt[16]; /* DAS rtx send count */
        ushort_t rtx_rec_cnt[16]; /* DAS rtx receive count */
        ushort_t rtx_skp_cnt[16]; /* DAS rtx skip count */
        ushort_t no_xmit[16];     /* DAS no xmit error count */
        ushort_t tx_time_err[16]; /* DAS xmit time error count */
        ushort_t overfow_err[16]; /* DAS buffer overflow error */
        uchar_t  last_command[64];/* Last remote command */
        ushort_t host_rtx_req;    /* Host rtx request count */
        ushort_t rem_pack_cnt;    /* remote buffer backlog */
        ushort_t host_rtx_skip;   /* Host rtx request skip count */
};
...
#define STE_TRIM(SP) \
TRIM((SP)->pkttype,11);TRIM((SP)->name,7);TRIM((SP)->up,2); \
TRIM((SP)->sens,11);TRIM((SP)->trg,2); TRIM((SP)->pick,2)
