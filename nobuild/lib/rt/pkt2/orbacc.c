...
Dbptr         *trp;
{
	...
	*trp = tr ;
    ...
    pkttime = 0.0 ; 
    for ( orbget (orb, ORBCURRENT, &pktid, srcname, &pkttime, &packet, &nbytes, &bufsize) ;
	  pktid != -1 && pkttime < req_endtime ;
	  orbget (orb, ORBNEXTT, &pktid, srcname, &pkttime, &packet, &nbytes, &bufsize)) {
