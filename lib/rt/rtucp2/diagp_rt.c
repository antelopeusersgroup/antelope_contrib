	...
	if ( regcomp(&dasmatch, &daspkt[0], REG_EXTENDED|REG_NOSUB) != 0)   {
	 ...
	 if (regcomp(&dcmatch, &dcpkt[0], REG_EXTENDED|REG_NOSUB) != 0)  {
    	...
    	if (argc < 2) {
         ...
         pfile_name = (char *) *argv;
	 argv++;
	 wantgain = atoi(*argv);
      
         /* build array of DAS name from 'Site' table from pkt.pf */ 
         Das_ID_toName = newarr(0);
         if(pfread( pfile_name, &pf) != 0)  {
    	      Tcl_AppendResult(interp, "can't read parameter file ", pfile_name, (char *) NULL);
	      return TCL_ERROR;
	 }
        ...
        /* build array of RTX parameter names from 'RTX_Stat' table from pkt.pf */
         if(RTX_ID == 0) RTX_ID = newarr(0);
/*
if(pfread( str, &pf) != 0)  {
...
*/
         Ste = pfget_tbl( pf, "RTX_Stat" );
         dasnum = maxtbl( Ste );
         if( dasnum <= 0 )  {
    	      Tcl_AppendResult(interp, "can't get RTX_Stat table ", (char *) NULL);
	      return TCL_ERROR;
	 }
         for( i = 0; i < dasnum; i++)  {
              str = (char *) gettbl(Ste, i);
              sscanf(str, STAT_SCS,  STAT_RVL(&stat));
              stanam = strdup( stat.name );
              sprintf( srcname, "%d\0", stat.id );
              setarr( RTX_ID, srcname, stanam );
        }
 
        /* build array of DAS parameter names from 'Das_Stat' table from pkt.pf */
         if(DAS_ID == 0) DAS_ID = newarr(0);
/*
         if(pfread( str, &pf) != 0)  {
    	      Tcl_AppendResult(interp, "can't read parameter file ", str, (char *) NULL);
	      return TCL_ERROR;
...
*/
         Ste = pfget_tbl( pf, "Das_Stat" );
         dasnum = maxtbl( Ste );
         if( dasnum <= 0 )  {
    	      Tcl_AppendResult(interp, "can't get Das_Stat table ", (char *) NULL);
	      return TCL_ERROR;
	 }
         for( i = 0; i < dasnum; i++)  {
              str = (char *) gettbl(Ste, i);
	...
	if( wantgain )  {
        ...
        dctcl = tclorb;
        /* build array of DC parameter names from 'DC_Stat' table from pkt.pf */
         pfile_name = (char *) *argv;
	 argv++;
      
         if(DC_ID == 0) DC_ID = newarr(0);
         if(pfread( pfile_name, &pf) != 0)  {
    	      Tcl_AppendResult(interp, "can't read parameter file ", pfile_name, (char *) NULL);
	      return TCL_ERROR;
	 }
         Ste = pfget_tbl( pf, "DC_Stat" );
         dasnum = maxtbl( Ste );
         if( dasnum <= 0 )  {
    	      Tcl_AppendResult(interp, "can't get DC_Stat table ", (char *) NULL);
	      return TCL_ERROR;
	 }
         for( i = 0; i < dasnum; i++)  {
              str = (char *) gettbl(Ste, i);
...
*/
	         while ( Tk_DoOneEvent (TK_DONT_WAIT) )  
                 ...
                 switch ( pkttype )  {
                     case 0xdaab:
                        if( !DAS_gain( gaintcl, pkey, ppkttime, ppacket, pnbytes )) return 0;
                              break;
	         ...
	         while ( Tk_DoOneEvent (TK_DONT_WAIT) )  
                 ...
                 if( !dc || !das )  { 
                     ...
                     if( dastcl->parcallback != NULL && !dc )  {
                      ...
                      switch ( pkttype )  {
                           case 0xdabc:
                              if( !BBA_DasPar( dastcl, pkey, ppkttime, ppacket, pnbytes )) return 0;
                              break;
                           case 0xdade:
...
*/
	          ...
	          while ( Tk_DoOneEvent (TK_DONT_WAIT) )  
                  ...
                  if( !dc || !das ) { 
                       ...
                       switch ( pkttype )  {
                           case 0xdacd:
                              if( !BBA_Dc( dctcl, dkey, dpkttime, dpacket, dnbytes )) return 0;
                              break;
                          default: break;
                       }
                   }
