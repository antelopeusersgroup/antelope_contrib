...
int difference_is_zero(float x, float y)
{
	...
	if(fabs(test) <= FLT_EPSILON ) return(1);
	return(0);
}
...
*/
Robust_statistics form_equations(int mode, Hypocenter current_location, 
	Tbl *attbl, Tbl *utbl,
	...
	float **A, float *b, float *r, float *w, float *reswt, int *nused)
{
	...
	double beta;
	int i,j,k,ii;
	int npar;  /* npar is the number of free location parameters.  It
	...
	for(i=0;i<natimes;++i)
	{
		...
		if(tto.time == TIME_INVALID)
		{
			...
			w[i] = 0.0;
			b[i] = 0.0;
			if(mode == ALL) for(k=0;k<npar;++k) A[i][k] = 0.0;
			...
			if(mode == ALL)
			{
				...
				if(!options.fix[k])
				{
					...
					case (2):
						...
						break;
					case (3):
					/* phase velocities don't ever
					depend upon the origin time */
						A[j][ii] = 0.0;
						A[j+1][ii] = 0.0;					}
					++ii;
				}
	...
	/* Use the interquartiles to determine the scale
	of errors in the weighted residual vector assuming the 
	residuals are normally distributed.  We first copy
	the residual vector to work because calc_statistics 
	sorts the input vector in place.  Note we throw out
	...
	*nused = ii;
	/* calc_statistics returns a bogus values when nused < 2
	We trap that situation by setting the error scale to max and
	assume the calling program will handle this problem correctly
	when it sees the value of *nused */
	...
	that depends on anything other than the error_scale.  Here we
	compute the ndata-th quantile of a normal distribution as 
	recommended by Chave and Thomson */
	if( ( (options.atime_residual_weight)==THOMSON)
	   	|| ( (options.slow_residual_weight) == THOMSON))
	{
		beta = normal_quantile(ndata,ndata);
	}
	/* We pass through this loop even if residual weighting is 
	turned off.  In that situation we just set the reswt values
	all to one.  Otherwise they are determined from the residual 
	values.  We need two loops, unfortunately, because we are
	allowing different schemes for arrival times and slowness.
	Because of this mess we are also going to scale the rows of
	the matrix A in the same pair of loops so we don't have to
	...
	for(i=0;i<natimes;++i)
	{
		...
		b[i] *= reswt[i];
		if(mode == ALL)
			for(k=0;k<npar;++k) A[i][k] *= ( w[i]*reswt[i]);
	}
	...
	Location_options options)
{
	...
	if( (ztrial > options.depth_ceiling) 
		&& (ztrial < options.depth_floor) ) return;

	/* block for events already at the ceiling or floor that
	would be adjusted outside these bounds */
	...
	else
	{
		...
		for(i=0;i<n;++i) dx[i] *= scale;

		/* when the step still is outside the bounds, we 
		leave the other coordinates rescaled, and set the depth
		the ceiling of floor as appropriate */
...
Written:  March 1996
*/ 
#define RADIUS_EARTH 6370.8 /* Radius of earth in km for lat-lon adjustment */

Hypocenter adjust_hypocenter(Hypocenter current_location, float *dx,
					int npar, Location_options o)
{
	...
	/* Now we convert correction for latitude and longitude
	using simple spherical formula.  Note, this assumes
	lat and lon are in degrees. */
	new_hypo.lon = current_location.lon 
	    +deg(cos(rad(current_location.lat))*(new_hypo.dx)/RADIUS_EARTH);
	if(new_hypo.lon > 180.0) new_hypo.lon -= 360.0;
	if(new_hypo.lon < -180.0) new_hypo.lon += 360.0;
	...
	/* Note we just copy the statistics.  For this reason
	the statistics listed are actually based on a linear
	extrapolation, which will not be totally accurate when
	...
	int m, int n, float damp, float *x)
{
	...
	/* Because we use relative damping, we have to find the 
	largest singular value, and rescale damp accordingly */
	smax = 0.0;
	for(i=0;i<n;++i) 
...
int pseudoinverse_solver(float **U, float *s, float **V, float *b,
...
int ggnloc (Hypocenter initial_location, 
	Tbl *attbl, Tbl *utbl,
	Location_options options,
	...
	Tbl **restbl)
{
	...
	float *btmp,*rtmp,*wtmp,*reswttmp;

	float *work;  /* work array the length of the data used in various 
			places */
	float dx[4];  /* hypocenter adjustment vector in local coordinates*/
	...
	Robust_statistics statistics, raw_statistics;
	int iz;  /* iz is set to the column of A corresponding to z
			This can vary because of fix options */
	double previous_wrms,new_wrms, test_wrms;  /* different weighted rms 
		statistics used in different contexts.  previous = previous iteration
		value, new_wrms = sets new value for current iteration, test = used
		in marquardt damping adjustments */
	double sum_reswt;  /* holds sum of residual weights used to calculate
				effective degrees of freedom*/
	...
	int i,j;
	float damp;
	double relative_rms_change,nrm2_dx;
	char *stack_mesg;
	int recenter;  /* set to one for recenter solutions */
	char res_output[80];  /* holds each line of residual information returned in restbl*/
	char *stmp;  /* holds pointer returned by strdup for each line formed in res_output */
	...
	/* Initialize output tbl.  We do this immediately since all are
	returned and the caller's responsibility to free */
	...
	*restbl = newtbl(0);
	...
	/* Alloc space for main working arrays.  Note we don't worry 
	about fix reducing necessary sizes because none of these
	arrays are that large.*/
	A = matrix(0,m-1,0,3);
	U = matrix(0,m-1,0,3);
	V = matrix(0,3,0,3);
	...
	do
	{
		int nsv_used,ndata_used,ndata_feq;
		Robust_statistics test;
		char message[80];

		hypo_history = (Hypocenter *) malloc(sizeof(Hypocenter));
		if(hypo_history==NULL) die(1,"Malloc error in location routine\n");
		...
		/* For recentered solutions we determine an origin time
		correction from a weighted median of the arrival times. 
		We handle that easily here by copying the first natimes 
		elements to the work array, and then calculating the 
		median.  Note this is NOT the same as the process used
		in form_equations above.  There we look at the whole vector
...
;
				...
				row weighting.  We only look at the statistics of
				the result vector to decide if we are improving or
				not */
			...
			/* The 2 here is needed because damp is adjusted
			at the bottom of the above loop.  Standard applicatoin
			of marquardt's method tries to continually reduce the
			damping constant.  Unless we do this, damp will only 
			get larger. */
		...
		is defined as those points for which neither of the weights
		for that datum are zero.  */
		...
		relative_rms_change = fabs(new_wrms - previous_wrms)
					/previous_wrms;
		++iteration;		
		/* We don't include origin time in convergence check.
		This may not always be smart, but because it has units
		it is dumb to include it in the same norm measure. 
		...
		if(nrm2_dx <= options.dx_convergence)
		{
			...
			stack_mesg = strdup(message);  /* strdup allocs new
							space before a copy.  WE
							need this here */
			pushtbl(*reason_converged, stack_mesg);
		}
	...
	/* Dont' forget to push the final hypocenter onto the
	convergence stack */
	hypo_history = (Hypocenter *) malloc(sizeof(Hypocenter));
	if(hypo_history==NULL) die(1,"Malloc error in location routine\n");
	copy_hypocenter(&current_location,hypo_history);
	...
	/* Now we form restbl contents.  The only tricky part is cleanly splitting array and
...
int lat_lon_grid_setup (Pf *pf, Point **pts, int *gridsize)
{
	char *s; /* string buffer */
	char err[80]="lat_lon_grid_setup can't find required parameter = ";
	double center_latitude, center_longitude, center_depth;
	double latitude_range, longitude_range, depth_range;
	int nlat, nlon, ndepths;
	double lat0, lon0, z0;
	double dlat, dlon, dz;
	Point *p;
	...
	for(k=0,n=0;k<ndepths;++k)
		for(j=0;j<nlon;++j)
			for(i=0;i<nlat;++i)
			{
				...
				if(p[n].lat > 180.0) p[n].lat -= 360.0;
				if(p[n].lat < -180.0) p[n].lat += 360.0;
...
int radial_grid_setup (Pf *pf, Point **pts, int *gridsize)
{
	char *s; /* string buffer */
	...
	{
		/* This is needed because otherwise we would hit 0.0
		azimuth twice when a full circle is specified. */
	...
	*pts = (Point *) calloc(*gridsize, sizeof(Point));
	...
	for(k=0,n=0;k<ndepths;++k)
		for(j=0;j<nr;++j)
		{
			...
			for(i=0;i<nphi;++i)
			{
				double rlat, rlon;  /* hold point in radians */
				double azimuth;
				azimuth = rad(phimin+dphi*((double)i) );
				latlon(center_latitude, center_longitude,
					distance, azimuth, &rlat, &rlon);
				/* It is not efficient to convert these
				to degrees, but it is more consistent with
				everything else to keep these in degrees */
	...
	int use_raw, Location_options options)
{
	...
	int i,j;
	double rms,wrms;
	Robust_statistics statistics, time_statistics;
	Arrival *a;
	int m, nused;
	double rmsmin;
	int imin;
	...
	/* Since we are working with a copy of the options structure, 
	we will just alter the control parameters we need that assure
	the form_equations procedure behaves as we want it to.  */
	for(i=0;i<4;++i) options.fix[i] = 0;
	...
	/* Grab the first arrival time listed in the table to get a 
