...
/* Computing MAX */
			...
			for (i__ = max(i__4,i__5); i__ <= i__3; ++i__) {
			    ...
			    s += ((d__1 = ab[i__4].r, abs(d__1)) + (d__2 = 
				    d_imag(&ab_ref(*kd + 1 + i__ - k, k)), 
				    abs(d__2))) * ((d__3 = x[i__5].r, abs(
				    d__3)) + (d__4 = d_imag(&x_ref(i__, j)), 
				    abs(d__4)));
/* L110: */
			}
			rwork[k] += s;
...
/* Computing MIN */
			...
			for (i__ = k; i__ <= i__5; ++i__) {
			    ...
			    i__4 = x_subscr(i__, j);
			    s += ((d__1 = ab[i__3].r, abs(d__1)) + (d__2 = 
				    d_imag(&ab_ref(i__ + 1 - k, k)), abs(d__2)
...
/* L160: */
		    ...
		    for (k = 1; k <= i__2; ++k) {
			i__5 = x_subscr(k, j);
			s = (d__1 = x[i__5].r, abs(d__1)) + (d__2 = d_imag(&
				x_ref(k, j)), abs(d__2));
/* Computing MIN */
			i__3 = *n, i__4 = k + *kd;
			i__5 = min(i__3,i__4);
