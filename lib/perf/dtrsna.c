...
/*              Real eigenvalue. */
		...
		prod = ddot_(n, &vr_ref(1, ks), &c__1, &vl_ref(1, ks), &c__1);
...
/*              Complex eigenvalue. */
		...
		prod1 = ddot_(n, &vr_ref(1, ks), &c__1, &vl_ref(1, ks), &c__1)
			;
		prod1 += ddot_(n, &vr_ref(1, ks + 1), &c__1, &vl_ref(1, ks + 
			1), &c__1);
		prod2 = ddot_(n, &vl_ref(1, ks), &c__1, &vr_ref(1, ks + 1), &
			c__1);
		prod2 -= ddot_(n, &vl_ref(1, ks + 1), &c__1, &vr_ref(1, ks), &
			c__1);
...
/*                 Triangularize the 2 by 2 block by unitary   
		    ...
		    mu = sqrt((d__1 = work_ref(1, 2), abs(d__1))) * sqrt((
			    d__2 = work_ref(2, 1), abs(d__2)));
		    delta = dlapy2_(&mu, &work_ref(2, 1));
		    cs = mu / delta;
...
/*              Estimate norm(inv(C')) */
		...
		kase = 0;
L50:
		dlacon_(&nn, &work_ref(1, *n + 2), &work_ref(1, *n + 4), &
			iwork[1], &est, &kase);
		if (kase != 0) {
		    if (kase == 1) {
			if (n2 == 1) {
...
/*                       Real eigenvalue: solve C'*x = scale*c. */
			    ...
			    i__2 = *n - 1;
			    dlaqtr_(&c_true, &c_true, &i__2, &work_ref(2, 2), 
...
/*                       Complex eigenvalue: solve   
			    ...
			    i__2 = *n - 1;
			    dlaqtr_(&c_true, &c_false, &i__2, &work_ref(2, 2),
...
/*                       Real eigenvalue: solve C*x = scale*c. */
			    ...
			    i__2 = *n - 1;
			    dlaqtr_(&c_false, &c_true, &i__2, &work_ref(2, 2),
...
/*                       Complex eigenvalue: solve   
			    ...
			    i__2 = *n - 1;
			    dlaqtr_(&c_false, &c_false, &i__2, &work_ref(2, 2)
				    , ldwork, &work_ref(1, *n + 1), &mu, &
				    scale, &work_ref(1, *n + 4), &work_ref(1, 
				    *n + 6), &ierr);
