...
/*        Determine ith pivot column and swap if necessary */
	...
	if (pvt != k) {
	    cswap_(m, &a_ref(1, pvt), &c__1, &a_ref(1, k), &c__1);
	    i__1 = k - 1;
	    cswap_(&i__1, &f_ref(pvt, 1), ldf, &f_ref(k, 1), ldf);
	    itemp = jpvt[pvt];
	    jpvt[pvt] = jpvt[k];
	    jpvt[k] = itemp;
	    vn1[pvt] = vn1[k];
...
/* L20: */
	    ...
	    cgemv_("No transpose", &i__1, &i__2, &q__1, &a_ref(rk, 1), lda, &
...
/*        Generate elementary reflector H(k). */
	...
	if (rk < *m) {
	    i__1 = *m - rk + 1;
	    clarfg_(&i__1, &a_ref(rk, k), &a_ref(rk + 1, k), &c__1, &tau[k]);
	} else {
	    clarfg_(&c__1, &a_ref(rk, k), &a_ref(rk, k), &c__1, &tau[k]);
	}
...
/*        Compute Kth column of F:   
	...
	if (k < *n) {
	    i__1 = *m - rk + 1;
	    i__2 = *n - k;
	    cgemv_("Conjugate transpose", &i__1, &i__2, &tau[k], &a_ref(rk, k 
...
/*        Incremental updating of F:   
	...
	if (k > 1) {
	    ...
	    cgemv_("Conjugate transpose", &i__1, &i__2, &q__1, &a_ref(rk, 1), 
		    lda, &a_ref(rk, k), &c__1, &c_b1, &auxv[1], &c__1);

	    i__1 = k - 1;
	    cgemv_("No transpose", n, &i__1, &c_b2, &f_ref(1, 1), ldf, &auxv[
...
/*        Update the current row of A:   
	...
	if (k < *n) {
	    ...
	    cgemm_("No transpose", "Conjugate transpose", &c__1, &i__1, &k, &
		    q__1, &a_ref(rk, 1), lda, &f_ref(k + 1, 1), ldf, &c_b2, &
		    a_ref(rk, k + 1), lda);
	}
...
L60:
	...
	i__1 = *m - rk;
	vn1[lsticc] = scnrm2_(&i__1, &a_ref(rk + 1, lsticc), &c__1);
