...
/* Computing MIN */
		i__2 = *kd, i__3 = j - 1;
		jlen = min(i__2,i__3);
		cnorm[j] = scasum_(&jlen, &ab_ref(*kd + 1 - jlen, j), &c__1);
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		jlen = min(i__2,i__3);
		if (jlen > 0) {
		    cnorm[j] = scasum_(&jlen, &ab_ref(2, j), &c__1);
		} else {
		    cnorm[j] = 0.f;
		}
/* L20: */
...
/*                    Compute the update   
   ...
   Computing MIN */
			...
			caxpy_(&jlen, &q__1, &ab_ref(*kd + 1 - jlen, j), &
				c__1, &x[j - jlen], &c__1);
			i__3 = j - 1;
...
/*                 Compute the update   
   ...
   Computing MIN */
			...
			q__2.r = -x[i__3].r, q__2.i = -x[i__3].i;
			q__1.r = tscal * q__2.r, q__1.i = tscal * q__2.i;
			caxpy_(&jlen, &q__1, &ab_ref(2, j), &c__1, &x[j + 1], 
				&c__1);
		    }
		    i__3 = *n - j;
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			cdotu_(&q__1, &jlen, &ab_ref(*kd + 1 - jlen, j), &
				c__1, &x[j - jlen], &c__1);
...
/* Computing MIN */
			...
			jlen = min(i__3,i__4);
			if (jlen > 1) {
			    cdotu_(&q__1, &jlen, &ab_ref(2, j), &c__1, &x[j + 
				    1], &c__1);
			    csumj.r = q__1.r, csumj.i = q__1.i;
			}
		    }
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			cdotc_(&q__1, &jlen, &ab_ref(*kd + 1 - jlen, j), &
				c__1, &x[j - jlen], &c__1);
			csumj.r = q__1.r, csumj.i = q__1.i;
		    } else {
/* Computing MIN */
			...
			jlen = min(i__3,i__4);
			if (jlen > 1) {
			    cdotc_(&q__1, &jlen, &ab_ref(2, j), &c__1, &x[j + 
				    1], &c__1);
			    csumj.r = q__1.r, csumj.i = q__1.i;
			}
		    }
...
/* Computing MIN */
			...
			i__3 = jlen;
			for (i__ = 1; i__ <= i__3; ++i__) {
			    r_cnjg(&q__4, &ab_ref(*kd + i__ - jlen, j));
