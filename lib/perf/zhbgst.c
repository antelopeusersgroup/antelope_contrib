...
/* L30: */
		...
		for (j = i__ - kbt; j <= i__1; ++j) {
		    ...
		    d_cnjg(&z__10, &bb_ref(k - i__ + kb1, i__));
...
/* Computing MAX */
		...
		for (j = max(i__1,i__2); j <= i__4; ++j) {
		    ...
		    i__2 = ab_subscr(j - k + ka1, k);
		    d_cnjg(&z__3, &bb_ref(k - i__ + kb1, i__));
...
/*              post-multiply X by inv(S(i)) */
		...
		i__3 = *n - m;
		d__1 = 1. / bii;
		zdscal_(&i__3, &d__1, &x_ref(m + 1, i__), &c__1);
		if (kbt > 0) {
		    i__3 = *n - m;
		    z__1.r = -1., z__1.i = 0.;
		    zgerc_(&i__3, &kbt, &z__1, &x_ref(m + 1, i__), &c__1, &
...
/*                 generate rotation to annihilate a(i,i-k+ka+1) */
		    ...
		    zlartg_(&ab_ref(k + 1, i__ - k + *ka), &ra1, &rwork[i__ - 
...
/*           generate rotations in 1st set to annihilate elements which   
	    ...
	    if (nrt > 0) {
		zlargv_(&nrt, &ab_ref(1, j2t), &inca, &work[j2t - m], &ka1, &
			rwork[j2t - m], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the right */
		...
		i__4 = *ka - 1;
		for (l = 1; l <= i__4; ++l) {
		    zlartv_(&nr, &ab_ref(ka1 - l, j2), &inca, &ab_ref(*ka - l,
...
/*              apply rotations in 1st set from both sides to diagonal   
		...
		zlar2v_(&nr, &ab_ref(ka1, j2), &ab_ref(ka1, j2 + 1), &ab_ref(*
...
/*           start applying rotations in 1st set from the left */
		...
		nrt = (*n - j2 + l) / ka1;
		if (nrt > 0) {
		    zlartv_(&nrt, &ab_ref(l, j2 + ka1 - l), &inca, &ab_ref(l 
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j2; i__2 < 0 ? j >= i__4 : j <= i__4; j += i__2) {
		    ...
		    zrot_(&i__1, &x_ref(m + 1, j), &c__1, &x_ref(m + 1, j + 1)
			    , &c__1, &rwork[j - m], &z__1);
/* L120: */
		}
	    }
...
/*           finish applying rotations in 2nd set from the left */
		...
		nrt = (*n - j2 + *ka + l) / ka1;
		if (nrt > 0) {
		    zlartv_(&nrt, &ab_ref(l, j2 - l + 1), &inca, &ab_ref(l + 
...
/*              generate rotations in 2nd set to annihilate elements   
		...
		zlargv_(&nr, &ab_ref(1, j2), &inca, &work[j2], &ka1, &rwork[
			j2], &ka1);
...
/*              post-multiply X by product of rotations in 2nd set */
		...
		for (j = j2; i__2 < 0 ? j >= i__3 : j <= i__3; j += i__2) {
		    i__4 = *n - m;
		    d_cnjg(&z__1, &work[j]);
		    zrot_(&i__4, &x_ref(m + 1, j), &c__1, &x_ref(m + 1, j + 1)
			    , &c__1, &rwork[j], &z__1);
/* L200: */
		}
	    }
...
/* L260: */
		...
		for (j = i__ - kbt; j <= i__2; ++j) {
		    ...
		    d_cnjg(&z__5, &ab_ref(i__ - k + 1, k));
		    z__4.r = bb[i__5].r * z__5.r - bb[i__5].i * z__5.i, 
			    z__4.i = bb[i__5].r * z__5.i + bb[i__5].i * 
			    z__5.r;
		    z__3.r = ab[i__1].r - z__4.r, z__3.i = ab[i__1].i - 
			    z__4.i;
		    d_cnjg(&z__7, &bb_ref(i__ - k + 1, k));
...
/*              post-multiply X by inv(S(i)) */
		...
		i__4 = *n - m;
		d__1 = 1. / bii;
		zdscal_(&i__4, &d__1, &x_ref(m + 1, i__), &c__1);
		if (kbt > 0) {
		    i__4 = *n - m;
		    z__1.r = -1., z__1.i = 0.;
		    i__3 = *ldbb - 1;
		    zgeru_(&i__4, &kbt, &z__1, &x_ref(m + 1, i__), &c__1, &
...
/*                 generate rotation to annihilate a(i-k+ka+1,i) */
		    ...
		    zlartg_(&ab_ref(ka1 - k, i__), &ra1, &rwork[i__ - k + *ka 
...
/*           generate rotations in 1st set to annihilate elements which   
	    ...
	    if (nrt > 0) {
		zlargv_(&nrt, &ab_ref(ka1, j2t - *ka), &inca, &work[j2t - m], 
			&ka1, &rwork[j2t - m], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the left */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    zlartv_(&nr, &ab_ref(l + 1, j2 - l), &inca, &ab_ref(l + 2,
...
/*              apply rotations in 1st set from both sides to diagonal   
		...
		zlar2v_(&nr, &ab_ref(1, j2), &ab_ref(1, j2 + 1), &ab_ref(2, 
...
/*           start applying rotations in 1st set from the right */
		...
		nrt = (*n - j2 + l) / ka1;
		if (nrt > 0) {
		    zlartv_(&nrt, &ab_ref(ka1 - l + 1, j2), &inca, &ab_ref(
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j2; i__3 < 0 ? j >= i__1 : j <= i__1; j += i__3) {
		    i__2 = *n - m;
		    zrot_(&i__2, &x_ref(m + 1, j), &c__1, &x_ref(m + 1, j + 1)
...
/*           finish applying rotations in 2nd set from the right */
		...
		nrt = (*n - j2 + *ka + l) / ka1;
		if (nrt > 0) {
		    zlartv_(&nrt, &ab_ref(ka1 - l + 1, j2 - *ka), &inca, &
...
/*              generate rotations in 2nd set to annihilate elements   
		...
		zlargv_(&nr, &ab_ref(ka1, j2 - *ka), &inca, &work[j2], &ka1, &
			rwork[j2], &ka1);
...
/* L510: */
		...
		for (j = k; j <= i__4; ++j) {
		    ...
		    d_cnjg(&z__5, &ab_ref(i__ - k + ka1, k));
		    z__4.r = bb[i__5].r * z__5.r - bb[i__5].i * z__5.i, 
			    z__4.i = bb[i__5].r * z__5.i + bb[i__5].i * 
			    z__5.r;
		    z__3.r = ab[i__2].r - z__4.r, z__3.i = ab[i__2].i - 
			    z__4.i;
		    d_cnjg(&z__7, &bb_ref(i__ - k + kb1, k));
		    i__6 = ab_subscr(i__ - j + ka1, j);
		    z__6.r = z__7.r * ab[i__6].r - z__7.i * ab[i__6].i, 
			    z__6.i = z__7.r * ab[i__6].i + z__7.i * ab[i__6]
			    .r;
		    ...
		    z__9.r = d__1 * bb[i__8].r, z__9.i = d__1 * bb[i__8].i;
		    d_cnjg(&z__10, &bb_ref(i__ - k + kb1, k));
		    z__8.r = z__9.r * z__10.r - z__9.i * z__10.i, z__8.i = 
			    z__9.r * z__10.i + z__9.i * z__10.r;
		    z__1.r = z__2.r + z__8.r, z__1.i = z__2.i + z__8.i;
		    ab[i__1].r = z__1.r, ab[i__1].i = z__1.i;
...
/* Computing MIN */
		...
		for (j = i__ + kbt + 1; j <= i__4; ++j) {
		    i__1 = ab_subscr(k - j + ka1, j);
		    i__2 = ab_subscr(k - j + ka1, j);
		    d_cnjg(&z__3, &bb_ref(i__ - k + kb1, k));
...
/*              post-multiply X by inv(S(i)) */
		...
		d__1 = 1. / bii;
		zdscal_(&nx, &d__1, &x_ref(1, i__), &c__1);
		if (kbt > 0) {
		    z__1.r = -1., z__1.i = 0.;
		    i__3 = *ldbb - 1;
		    zgeru_(&nx, &kbt, &z__1, &x_ref(1, i__), &c__1, &bb_ref(*
...
/*                 generate rotation to annihilate a(i+k-ka-1,i) */
		    ...
		    zlartg_(&ab_ref(k + 1, i__), &ra1, &rwork[i__ + k - *ka], 
			    &work[i__ + k - *ka], &ra);
...
/*           generate rotations in 1st set to annihilate elements which   
	    ...
	    if (nrt > 0) {
		zlargv_(&nrt, &ab_ref(1, j1 + *ka), &inca, &work[j1], &ka1, &
			rwork[j1], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the left */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    zlartv_(&nr, &ab_ref(ka1 - l, j1 + l), &inca, &ab_ref(*ka 
...
/*              apply rotations in 1st set from both sides to diagonal   
		...
		zlar2v_(&nr, &ab_ref(ka1, j1), &ab_ref(ka1, j1 - 1), &ab_ref(*
...
/*           start applying rotations in 1st set from the right */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    zlartv_(&nrt, &ab_ref(l, j1t), &inca, &ab_ref(l + 1, j1t 
...
/*              post-multiply X by product of rotations in 1st set */
		...
		for (j = j1; i__4 < 0 ? j >= i__1 : j <= i__1; j += i__4) {
		    zrot_(&nx, &x_ref(1, j), &c__1, &x_ref(1, j - 1), &c__1, &
			    rwork[j], &work[j]);
/* L600: */
		}
	    }
...
/*           finish applying rotations in 2nd set from the right */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    zlartv_(&nrt, &ab_ref(l, j1t + *ka), &inca, &ab_ref(l + 1,
...
/*              generate rotations in 2nd set to annihilate elements   
		...
		zlargv_(&nr, &ab_ref(1, j1 + *ka), &inca, &work[m - *kb + j1],
...
/* L740: */
		...
		for (j = k; j <= i__3; ++j) {
		    ...
		    d_cnjg(&z__5, &ab_ref(k - i__ + 1, i__));
		    z__4.r = bb[i__5].r * z__5.r - bb[i__5].i * z__5.i, 
			    z__4.i = bb[i__5].r * z__5.i + bb[i__5].i * 
			    z__5.r;
		    z__3.r = ab[i__2].r - z__4.r, z__3.i = ab[i__2].i - 
			    z__4.i;
		    d_cnjg(&z__7, &bb_ref(k - i__ + 1, i__));
		    i__6 = ab_subscr(j - i__ + 1, i__);
		    z__6.r = z__7.r * ab[i__6].r - z__7.i * ab[i__6].i, 
			    z__6.i = z__7.r * ab[i__6].i + z__7.i * ab[i__6]
			    .r;
		    ...
		    z__9.r = d__1 * bb[i__8].r, z__9.i = d__1 * bb[i__8].i;
		    d_cnjg(&z__10, &bb_ref(k - i__ + 1, i__));
		    z__8.r = z__9.r * z__10.r - z__9.i * z__10.i, z__8.i = 
			    z__9.r * z__10.i + z__9.i * z__10.r;
		    z__1.r = z__2.r + z__8.r, z__1.i = z__2.i + z__8.i;
		    ab[i__1].r = z__1.r, ab[i__1].i = z__1.i;
...
/* Computing MIN */
		...
		for (j = i__ + kbt + 1; j <= i__3; ++j) {
		    i__1 = ab_subscr(j - k + 1, k);
		    i__2 = ab_subscr(j - k + 1, k);
		    d_cnjg(&z__3, &bb_ref(k - i__ + 1, i__));
...
/*              post-multiply X by inv(S(i)) */
		...
		if (kbt > 0) {
		    z__1.r = -1., z__1.i = 0.;
		    zgerc_(&nx, &kbt, &z__1, &x_ref(1, i__), &c__1, &bb_ref(2,
...
/*                 generate rotation to annihilate a(i,i+k-ka-1) */
		    ...
		    zlartg_(&ab_ref(ka1 - k, i__ + k - *ka), &ra1, &rwork[i__ 
...
/*           generate rotations in 1st set to annihilate elements which   
	    ...
	    if (nrt > 0) {
		zlargv_(&nrt, &ab_ref(ka1, j1), &inca, &work[j1], &ka1, &
			rwork[j1], &ka1);
	    }
	    if (nr > 0) {
...
/*              apply rotations in 1st set from the right */
		...
		i__1 = *ka - 1;
		for (l = 1; l <= i__1; ++l) {
		    zlartv_(&nr, &ab_ref(l + 1, j1), &inca, &ab_ref(l + 2, j1 
...
/*              apply rotations in 1st set from both sides to diagonal   
		...
		zlar2v_(&nr, &ab_ref(1, j1), &ab_ref(1, j1 - 1), &ab_ref(2, 
...
/*           start applying rotations in 1st set from the left */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    zlartv_(&nrt, &ab_ref(ka1 - l + 1, j1t - ka1 + l), &inca, 
...
/*           finish applying rotations in 2nd set from the left */
		...
		j1t = j2 - (nrt - 1) * ka1;
		if (nrt > 0) {
		    zlartv_(&nrt, &ab_ref(ka1 - l + 1, j1t + l - 1), &inca, &
...
/*              generate rotations in 2nd set to annihilate elements   
		...
		zlargv_(&nr, &ab_ref(ka1, j1), &inca, &work[m - *kb + j1], &
			ka1, &rwork[m - *kb + j1], &ka1);
