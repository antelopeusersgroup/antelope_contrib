...
/* Computing MAX */
			...
			for (i__ = max(i__3,i__4); i__ <= i__5; ++i__) {
			    work[i__] += (r__1 = ab_ref(*kd + 1 + i__ - k, k),
				     dabs(r__1)) * xk;
/* L30: */
			}
/* L40: */
...
/* Computing MIN */
			...
			for (i__ = k; i__ <= i__4; ++i__) {
			    work[i__] += (r__1 = ab_ref(i__ + 1 - k, k), dabs(
				    r__1)) * xk;
/* L70: */
			}
/* L80: */
...
/* Computing MAX */
			...
			for (i__ = max(i__4,i__5); i__ <= i__3; ++i__) {
			    s += (r__1 = ab_ref(*kd + 1 + i__ - k, k), dabs(
				    r__1)) * (r__2 = x_ref(i__, j), dabs(r__2)
				    );
/* L110: */
			}
			work[k] += s;
...
/* Computing MIN */
			...
			for (i__ = k; i__ <= i__5; ++i__) {
			    s += (r__1 = ab_ref(i__ + 1 - k, k), dabs(r__1)) *
				     (r__2 = x_ref(i__, j), dabs(r__2));
/* L150: */
			}
			work[k] += s;
/* L160: */
