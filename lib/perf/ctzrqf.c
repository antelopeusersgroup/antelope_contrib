...
/*           Use a Householder reflection to zero the kth row of A.   
	    ...
	    i__1 = a_subscr(k, k);
	    r_cnjg(&q__1, &a_ref(k, k));
	    a[i__1].r = q__1.r, a[i__1].i = q__1.i;
	    i__1 = *n - *m;
	    clacgv_(&i__1, &a_ref(k, m1), lda);
	    i__1 = a_subscr(k, k);
	    alpha.r = a[i__1].r, alpha.i = a[i__1].i;
	    i__1 = *n - *m + 1;
	    clarfg_(&i__1, &alpha, &a_ref(k, m1), lda, &tau[k]);
	    i__1 = a_subscr(k, k);
...
/*              We now perform the operation  A := A*P( k )'.   
		...
		i__1 = k - 1;
		ccopy_(&i__1, &a_ref(1, k), &c__1, &tau[1], &c__1);

/*              Form   w = a( k ) + B*z( k )  in TAU. */

		i__1 = k - 1;
		i__2 = *n - *m;
		cgemv_("No transpose", &i__1, &i__2, &c_b1, &a_ref(1, m1), 
...
/*              Now form  a( k ) := a( k ) - conjg(tau)*w   
		...
		caxpy_(&i__1, &q__1, &tau[1], &c__1, &a_ref(1, k), &c__1);
		i__1 = k - 1;
		i__2 = *n - *m;
		r_cnjg(&q__2, &tau[k]);
		q__1.r = -q__2.r, q__1.i = -q__2.i;
		cgerc_(&i__1, &i__2, &q__1, &tau[1], &c__1, &a_ref(k, m1), 
			lda, &a_ref(1, m1), lda);
	    }
/* L20: */
	}
