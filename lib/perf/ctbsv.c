...
/* L20: */
		...
		for (j = *n; j >= 1; --j) {
			...
			if (nounit) {
			    i__1 = jx;
			    c_div(&q__1, &x[jx], &a_ref(kplus1, j));
			    x[i__1].r = q__1.r, x[i__1].i = q__1.i;
			}
			i__1 = jx;
			temp.r = x[i__1].r, temp.i = x[i__1].i;
...
/* L90: */
			...
			if (nounit) {
			    c_div(&q__1, &temp, &a_ref(kplus1, j));
			    temp.r = q__1.r, temp.i = q__1.i;
			}
		    } else {
/* Computing MAX */
