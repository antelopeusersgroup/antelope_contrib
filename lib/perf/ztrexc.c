...
/*        Determine the transformation to perform the interchange. */
	...
	zlartg_(&t_ref(k, k + 1), &z__1, &cs, &sn, &temp);

/*        Apply transformation to the matrix T. */

	if (k + 2 <= *n) {
	    i__3 = *n - k - 1;
	    zrot_(&i__3, &t_ref(k, k + 2), ldt, &t_ref(k + 1, k + 2), ldt, &
		    cs, &sn);
	}
	i__3 = k - 1;
	d_cnjg(&z__1, &sn);
	zrot_(&i__3, &t_ref(1, k), &c__1, &t_ref(1, k + 1), &c__1, &cs, &z__1)
		;
...
/*           Accumulate transformation in the matrix Q. */
	    ...
	    d_cnjg(&z__1, &sn);
	    zrot_(n, &q_ref(1, k), &c__1, &q_ref(1, k + 1), &c__1, &cs, &z__1)
		    ;
	}
