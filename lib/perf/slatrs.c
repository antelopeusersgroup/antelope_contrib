...
/*           A is upper triangular. */
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = j - 1;
		cnorm[j] = sasum_(&i__2, &a_ref(1, j), &c__1);
/* L10: */
	    }
	} else {
...
/*           A is lower triangular. */
	    ...
	    for (j = 1; j <= i__1; ++j) {
		i__2 = *n - j;
		cnorm[j] = sasum_(&i__2, &a_ref(j + 1, j), &c__1);
/* L20: */
	    }
	    cnorm[*n] = 0.f;
	}
...
/*                    Compute the update   
			...
			i__3 = j - 1;
			r__1 = -x[j] * tscal;
			saxpy_(&i__3, &r__1, &a_ref(1, j), &c__1, &x[1], &
				c__1);
			i__3 = j - 1;
...
/*                    Compute the update   
			...
			i__3 = *n - j;
			r__1 = -x[j] * tscal;
			saxpy_(&i__3, &r__1, &a_ref(j + 1, j), &c__1, &x[j + 
				1], &c__1);
			i__3 = *n - j;
...
/*                 If the scaling needed for A in the dot product is 1,   
		    ...
		    if (upper) {
			i__3 = j - 1;
			sumj = sdot_(&i__3, &a_ref(1, j), &c__1, &x[1], &c__1)
				;
		    } else if (j < *n) {
			i__3 = *n - j;
			sumj = sdot_(&i__3, &a_ref(j + 1, j), &c__1, &x[j + 1]
				, &c__1);
		    }
		} else {
