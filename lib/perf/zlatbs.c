...
/* Computing MIN */
		i__2 = *kd, i__3 = j - 1;
		jlen = min(i__2,i__3);
		cnorm[j] = dzasum_(&jlen, &ab_ref(*kd + 1 - jlen, j), &c__1);
/* L10: */
	    }
	} else {
...
/* Computing MIN */
		...
		jlen = min(i__2,i__3);
		if (jlen > 0) {
		    cnorm[j] = dzasum_(&jlen, &ab_ref(2, j), &c__1);
		} else {
		    cnorm[j] = 0.;
		}
/* L20: */
...
/*                    Compute the update   
   ...
   Computing MIN */
			...
			zaxpy_(&jlen, &z__1, &ab_ref(*kd + 1 - jlen, j), &
				c__1, &x[j - jlen], &c__1);
			i__3 = j - 1;
...
/*                 Compute the update   
   ...
   Computing MIN */
			...
			z__2.r = -x[i__3].r, z__2.i = -x[i__3].i;
			z__1.r = tscal * z__2.r, z__1.i = tscal * z__2.i;
			zaxpy_(&jlen, &z__1, &ab_ref(2, j), &c__1, &x[j + 1], 
				&c__1);
		    }
		    i__3 = *n - j;
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			zdotu_(&z__1, &jlen, &ab_ref(*kd + 1 - jlen, j), &
				c__1, &x[j - jlen], &c__1);
...
/* Computing MIN */
			...
			jlen = min(i__3,i__4);
			if (jlen > 1) {
			    zdotu_(&z__1, &jlen, &ab_ref(2, j), &c__1, &x[j + 
				    1], &c__1);
			    csumj.r = z__1.r, csumj.i = z__1.i;
			}
		    }
...
/* Computing MIN */
			i__3 = *kd, i__4 = j - 1;
			jlen = min(i__3,i__4);
			zdotc_(&z__1, &jlen, &ab_ref(*kd + 1 - jlen, j), &
				c__1, &x[j - jlen], &c__1);
			csumj.r = z__1.r, csumj.i = z__1.i;
		    } else {
/* Computing MIN */
			...
			jlen = min(i__3,i__4);
			if (jlen > 1) {
			    zdotc_(&z__1, &jlen, &ab_ref(2, j), &c__1, &x[j + 
				    1], &c__1);
			    csumj.r = z__1.r, csumj.i = z__1.i;
			}
		    }
...
/* Computing MIN */
			...
			i__3 = jlen;
			for (i__ = 1; i__ <= i__3; ++i__) {
			    d_cnjg(&z__4, &ab_ref(*kd + i__ - jlen, j));
