...
/*              Form the triangular factor of the block reflector   
		...
		i__3 = *n - *k + i__ + ib - 1;
		slarft_("Backward", "Rowwise", &i__3, &ib, &a_ref(ii, 1), lda,
...
/*              Apply H' to A(1:m-k+i-1,1:n-k+i+ib-1) from the right */
		...
		slarfb_("Right", "Transpose", "Backward", "Rowwise", &i__3, &
			i__4, &ib, &a_ref(ii, 1), lda, &work[1], &ldwork, &a[
			a_offset], lda, &work[ib + 1], &ldwork);
	    }

/*           Apply H' to columns 1:n-k+i+ib-1 of current block */

	    i__3 = *n - *k + i__ + ib - 1;
	    sorgr2_(&ib, &i__3, &ib, &a_ref(ii, 1), lda, &tau[i__], &work[1], 
		    &iinfo);
