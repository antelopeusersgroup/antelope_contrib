...
/*        Loop until stopping criterion is satisfied.   
	...
	z__1.r = -1., z__1.i = 0.;
	zgbmv_(trans, n, n, kl, ku, &z__1, &ab[ab_offset], ldab, &x_ref(1, j),
...
/* Computing MIN */
		...
		for (i__ = max(i__3,i__4); i__ <= i__5; ++i__) {
		    ...
		    rwork[i__] += ((d__1 = ab[i__3].r, abs(d__1)) + (d__2 = 
			    d_imag(&ab_ref(kk + i__, k)), abs(d__2))) * xk;
/* L40: */
		}
/* L50: */
	    }
...
/* Computing MIN */
		...
		for (i__ = max(i__5,i__3); i__ <= i__4; ++i__) {
		    ...
		    i__3 = x_subscr(i__, j);
		    s += ((d__1 = ab[i__5].r, abs(d__1)) + (d__2 = d_imag(&
			    ab_ref(kk + i__, k)), abs(d__2))) * ((d__3 = x[
			    i__3].r, abs(d__3)) + (d__4 = d_imag(&x_ref(i__, 
			    j)), abs(d__4)));
/* L60: */
		}
		rwork[k] += s;
