...
/*  -- LAPACK routine (version 3.0) --   
    ...
    static integer j, nbmin, minmn, minws;
...
/*     Move initial columns up front. */
	...
	if (jpvt[j] != 0) {
	    if (j != nfxd) {
		sswap_(m, &a_ref(1, j), &c__1, &a_ref(1, nfxd), &c__1);
		jpvt[j] = jpvt[nfxd];
		jpvt[nfxd] = j;
	    } else {
		jpvt[j] = j;
...
/*        Initialize partial column norms. The first N elements of work   
	...
	for (j = nfxd + 1; j <= i__1; ++j) {
	    work[j] = snrm2_(&sm, &a_ref(nfxd + 1, j), &c__1);
	    work[*n + j] = work[j];
/* L20: */
	}
...
/*              Factorize JB columns among columns J:N. */
		...
		i__2 = j - 1;
		i__3 = *n - j + 1;
		slaqps_(m, &i__1, &i__2, &jb, &fjb, &a_ref(1, j), lda, &jpvt[
...
/*        Use unblocked code to factor the last or only block. */
	...
	if (j <= minmn) {
	    i__1 = *n - j + 1;
	    i__2 = j - 1;
	    slaqp2_(m, &i__1, &i__2, &a_ref(1, j), lda, &jpvt[j], &tau[j], &
