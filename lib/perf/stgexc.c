...
/*     Determine the first row of the specified block and find out   
    ...
    if (*ifst > 1) {
	if (a_ref(*ifst, *ifst - 1) != 0.f) {
	    --(*ifst);
	}
    }
    nbf = 1;
    if (*ifst < *n) {
	if (a_ref(*ifst + 1, *ifst) != 0.f) {
	    nbf = 2;
	}
    }
...
/*     Determine the first row of the final block   
    ...
    if (*ilst > 1) {
	if (a_ref(*ilst, *ilst - 1) != 0.f) {
	    --(*ilst);
	}
    }
    nbl = 1;
    if (*ilst < *n) {
	if (a_ref(*ilst + 1, *ilst) != 0.f) {
	    nbl = 2;
	}
    }
    if (*ifst == *ilst) {
...
/*           Current block either 1-by-1 or 2-by-2. */
	    ...
	    nbnext = 1;
	    if (here + nbf + 1 <= *n) {
		if (a_ref(here + nbf + 1, here + nbf) != 0.f) {
		    nbnext = 2;
		}
	    }
