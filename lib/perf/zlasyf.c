...
/*        Copy column K of A to column KW of W and update it */
	...
	zcopy_(&k, &a_ref(1, k), &c__1, &w_ref(1, kw), &c__1);
	if (k < *n) {
	    i__1 = *n - k;
	    z__1.r = -1., z__1.i = 0.;
...
/*        Determine rows and columns to be interchanged and whether   
	...
	i__1 = w_subscr(k, kw);
	absakk = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w_ref(k, kw))
		, abs(d__2));
...
/*              Copy column IMAX to column KW-1 of W and update it */
		...
		zcopy_(&imax, &a_ref(1, imax), &c__1, &w_ref(1, kw - 1), &
			c__1);
		i__1 = k - imax;
...
/*                 no interchange, use 1-by-1 pivot block */
		    ...
		    if ((d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w_ref(
			    imax, kw - 1)), abs(d__2)) >= alpha * rowmax) {
...
/*              Copy non-updated column KK to column KP */
		...
		zcopy_(&i__1, &a_ref(kp + 1, kk), &c__1, &a_ref(kp, kp + 1), 
			lda);
		zcopy_(&kp, &a_ref(1, kk), &c__1, &a_ref(1, kp), &c__1);

/*              Interchange rows KK and KP in last KK columns of A and W */

		i__1 = *n - kk + 1;
		zswap_(&i__1, &a_ref(kk, kk), lda, &a_ref(kp, kk), lda);
		i__1 = *n - kk + 1;
...
/*        Copy column K of A to column K of W and update it */
	...
	i__1 = *n - k + 1;
	zcopy_(&i__1, &a_ref(k, k), &c__1, &w_ref(k, k), &c__1);
	i__1 = *n - k + 1;
	i__2 = k - 1;
	z__1.r = -1., z__1.i = 0.;
...
/*        Determine rows and columns to be interchanged and whether   
	...
	i__1 = w_subscr(k, k);
	absakk = (d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w_ref(k, k)),
		 abs(d__2));
...
/*              Copy column IMAX to column K+1 of W and update it */
		...
		i__1 = *n - imax + 1;
		zcopy_(&i__1, &a_ref(imax, imax), &c__1, &w_ref(imax, k + 1), 
			&c__1);
		i__1 = *n - k + 1;
		i__2 = k - 1;
		z__1.r = -1., z__1.i = 0.;
...
/*                 no interchange, use 1-by-1 pivot block */
		    ...
		    if ((d__1 = w[i__1].r, abs(d__1)) + (d__2 = d_imag(&w_ref(
			    imax, k + 1)), abs(d__2)) >= alpha * rowmax) {
...
/*              Copy non-updated column KK to column KP */
		...
		a[i__1].r = a[i__2].r, a[i__1].i = a[i__2].i;
		i__1 = kp - k - 1;
		zcopy_(&i__1, &a_ref(k + 1, kk), &c__1, &a_ref(kp, k + 1), 
			lda);
		i__1 = *n - kp + 1;
		zcopy_(&i__1, &a_ref(kp, kk), &c__1, &a_ref(kp, kp), &c__1);

/*              Interchange rows KK and KP in first KK columns of A and W */

		zswap_(&kk, &a_ref(kk, 1), lda, &a_ref(kp, 1), lda);
