...
L20:
    ...
    i__1 = *n - k + 1;
    sswap_(&i__1, &a_ref(j, k), lda, &a_ref(m, k), lda);
...
L140:
	...
	for (j = k; j <= i__2; ++j) {
	    ...
	    c__ += (r__1 = a_ref(j, i__), dabs(r__1));
	    r__ += (r__1 = a_ref(i__, j), dabs(r__1));
L150:
	    ;
	}
	ica = isamax_(&l, &a_ref(1, i__), &c__1);
	ca = (r__1 = a_ref(ica, i__), dabs(r__1));
	i__2 = *n - k + 1;
	ira = isamax_(&i__2, &a_ref(i__, k), lda);
	ra = (r__1 = a_ref(i__, ira + k - 1), dabs(r__1));
...
L190:
	...
	i__2 = *n - k + 1;
	sscal_(&i__2, &g, &a_ref(i__, k), lda);
	sscal_(&l, &f, &a_ref(1, i__), &c__1);
