...
L160:
    ...
    i__1 = *n - k + 1;
    cswap_(&i__1, &a_ref(i__, k), lda, &a_ref(m, k), lda);
    i__1 = *n - k + 1;
    cswap_(&i__1, &b_ref(i__, k), ldb, &b_ref(m, k), ldb);
...
L170:
    ...
    cswap_(&l, &a_ref(1, j), &c__1, &a_ref(1, m), &c__1);
    cswap_(&l, &b_ref(1, j), &c__1, &b_ref(1, m), &c__1);
...
/*     Compute right side vector in resulting linear equations */
	...
	for (j = *ilo; j <= i__2; ++j) {
	    ...
	    i__3 = a_subscr(i__, j);
	    r__3 = (r__1 = a[i__3].r, dabs(r__1)) + (r__2 = r_imag(&a_ref(i__,
		     j)), dabs(r__2));
	    ta = r_lg10(&r__3) / basl;
...
L210:
	    ...
	    i__3 = b_subscr(i__, j);
	    r__3 = (r__1 = b[i__3].r, dabs(r__1)) + (r__2 = r_imag(&b_ref(i__,
		     j)), dabs(r__2));
	    tb = r_lg10(&r__3) / basl;
...
L350:
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
	irab = icamax_(&i__2, &a_ref(i__, *ilo), lda);
	rab = c_abs(&a_ref(i__, irab + *ilo - 1));
	i__2 = *n - *ilo + 1;
	irab = icamax_(&i__2, &b_ref(i__, *ilo), lda);
/* Computing MAX */
	r__1 = rab, r__2 = c_abs(&b_ref(i__, irab + *ilo - 1));
	rab = dmax(r__1,r__2);
	r__1 = rab + sfmin;
...
/* Computing MIN */
	...
	icab = icamax_(ihi, &a_ref(1, i__), &c__1);
	cab = c_abs(&a_ref(icab, i__));
	icab = icamax_(ihi, &b_ref(1, i__), &c__1);
/* Computing MAX */
	r__1 = cab, r__2 = c_abs(&b_ref(icab, i__));
	cab = dmax(r__1,r__2);
	r__1 = cab + sfmin;
...
/*     Row scaling of matrices A and B */
    ...
    for (i__ = *ilo; i__ <= i__1; ++i__) {
	i__2 = *n - *ilo + 1;
	csscal_(&i__2, &lscale[i__], &a_ref(i__, *ilo), lda);
	i__2 = *n - *ilo + 1;
	csscal_(&i__2, &lscale[i__], &b_ref(i__, *ilo), ldb);
/* L370: */
    }
...
/*     Column scaling of matrices A and B */
    ...
    i__1 = *ihi;
    for (j = *ilo; j <= i__1; ++j) {
	csscal_(ihi, &rscale[j], &a_ref(1, j), &c__1);
	csscal_(ihi, &rscale[j], &b_ref(1, j), &c__1);
/* L380: */
    }
