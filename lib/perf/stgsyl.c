...
/*     Determine optimal block sizes MB and NB */
	...
	i__1 = *n;
	for (j = 1; j <= i__1; ++j) {
	    scopy_(m, &c_b14, &c__0, &c___ref(1, j), &c__1);
	    scopy_(m, &c_b14, &c__0, &f_ref(1, j), &c__1);
/* L10: */
	}
...
/*           Solve (I, J)-subsystem   
	    ...
	    *scale = 1.f;
		...
		for (i__ = p; i__ >= 1; --i__) {
		    ...
		    mb = ie - is + 1;
		    ppqq = 0;
		    stgsy2_(trans, &ifunc, &mb, &nb, &a_ref(is, is), lda, &
...
/* L80: */
			...
			for (k = js; k <= i__3; ++k) {
			    i__4 = is - 1;
			    sscal_(&i__4, &scaloc, &c___ref(1, k), &c__1);
			    i__4 = is - 1;
			    sscal_(&i__4, &scaloc, &f_ref(1, k), &c__1);
/* L90: */
			}
			i__3 = je;
			for (k = js; k <= i__3; ++k) {
			    i__4 = *m - ie;
			    sscal_(&i__4, &scaloc, &c___ref(ie + 1, k), &c__1)
				    ;
			    i__4 = *m - ie;
			    sscal_(&i__4, &scaloc, &f_ref(ie + 1, k), &c__1);
/* L100: */
			}
			i__3 = *n;
			for (k = je + 1; k <= i__3; ++k) {
...
/*                 Substitute R(I, J) and L(I, J) into remaining   
		    ...
		    if (i__ > 1) {
			i__3 = is - 1;
			sgemm_("N", "N", &i__3, &nb, &mb, &c_b53, &a_ref(1, 
				is), lda, &c___ref(is, js), ldc, &c_b54, &
				c___ref(1, js), ldc);
			i__3 = is - 1;
			sgemm_("N", "N", &i__3, &nb, &mb, &c_b53, &d___ref(1, 
				is), ldd, &c___ref(is, js), ldc, &c_b54, &
				f_ref(1, js), ldf);
		    }
		    if (j < q) {
			i__3 = *n - je;
			sgemm_("N", "N", &mb, &i__3, &nb, &c_b54, &f_ref(is, 
...
/*        Solve transposed (I, J)-subsystem   
	...
	*scale = 1.f;
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
		...
		je = iwork[j + 1] - 1;
		nb = je - js + 1;
		stgsy2_(trans, &ifunc, &mb, &nb, &a_ref(is, is), lda, &b_ref(
...
/*              Substitute R(I, J) and L(I, J) into remaining equation. */
		...
		if (j > p + 2) {
		    i__3 = js - 1;
		    sgemm_("N", "T", &mb, &i__3, &nb, &c_b54, &c___ref(is, js)
			    , ldc, &b_ref(1, js), ldb, &c_b54, &f_ref(is, 1), 
			    ldf);
		    i__3 = js - 1;
		    sgemm_("N", "T", &mb, &i__3, &nb, &c_b54, &f_ref(is, js), 
			    ldf, &e_ref(1, js), lde, &c_b54, &f_ref(is, 1), 
			    ldf);
		}
		if (i__ < p) {
		    i__3 = *m - ie;
		    sgemm_("T", "N", &i__3, &nb, &mb, &c_b53, &a_ref(is, ie + 
			    1), lda, &c___ref(is, js), ldc, &c_b54, &c___ref(
			    ie + 1, js), ldc);
		    i__3 = *m - ie;
		    sgemm_("T", "N", &i__3, &nb, &mb, &c_b53, &d___ref(is, ie 
