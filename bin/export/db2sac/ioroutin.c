...
int             n;
{
	...
	if (gethead (&head, file_pt) == 1) {
	    if (fseek (file_pt, (head.record.ndata) * (size (&head)), 1) == -1) {
		ierr = -2;	       /* bad seek */
		ah_errno = AE_RHED;
		return (ierr);
	    }
 ...
 */
int
gogetrecord (n, head, array, file_pt)
int             n;
ahhed          *head;
char           *array;
FILE           *file_pt;
...
/* check the length of the input array */
    ...
    if (in > LOGENT) {		       /* entry length too long-----truncate
				        * it */
	err = (-2);
	*(char_pt + LOGENT - 1) = ';';
	*(char_pt + LOGENT) = '\0';
...
void
ah_error (s1, s2, status)	       /* print ah format error message and
				        * die */
char           *s1,
               *s2;
int             status;
 ...
 */
int
xdr_getrecord2 (head, array, xdrs)
ahhed          *head;
char          **array;
XDR            *xdrs;
{
 ...
 */
int
xdr_gogetrecord (n, head, array, xdrs)
int             n;
ahhed          *head;
char           *array;
XDR            *xdrs;
