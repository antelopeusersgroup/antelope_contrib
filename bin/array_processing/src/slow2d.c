/* 
This program is meant to compute the slowness power map in 2-D for a spatial
array of sensors.  It uses inefficient time-domain algorithms to compute the 
beam power at each slowness point in 2-D by delaying the individual traces
according to beam direction and then summing them.  Results for different
center frequencies can be computed by varying the "filter" specification.
An output 2-D grid of power values is written to a file indexed by the fkgrid
CSS3.0 table.

Usage: slow2d tstart slen wlen filter sr kmax nk ampfunc beamtype channame 
       arrayname refsta dbname dir dfile

where

  tstart = start time of signal in epoch seconds
  slen = total signal length to be analyzed in seconds
  ...
  channame = channel name to be assigned to slowness grid  (maximum of 8 
             ...
             grid identifiers
  arrayname = name of array (maximum of 6 characters)
...
modified 12/07/2007 -- added ppower to variables output to fkgrid table, where
                       ppower here defined as the log10(peak of wavenumber grid)
*/
...
{
  int      ireturn,n,len,nbytes,i,j,index,nk,nkx,nky;
  int      iret,idec,npts,nptso;
  int      mpts[MXSTA];
  long int ns,nw,nz,noff,nkd,nwin;
  double   kxmin,kxmax,kxinc,kymin,kymax,kyinc,kmin,kmax,kinc,rdec,
           kopt,thetaopt,kxbest,kybest,slen,wlen,lat[MXSTA],lon[MXSTA],
           reflon,reflat,sr,sro,distance,azimuth,
           del,delo,tstart,tend,ts[MXSTA],te[MXSTA],wstart,wend,mean,stdv,calib,
           semin,semax,snmin,snmax,fnyq,zero,minamp,maxamp,ppower;
  char     asta[7],channame[9],refsta[7],searchExpr[200],corner[7];
  char     sta[MXSTA][7],chan[MXSTA][9];
  char     *arrayfile,*arrayname,*dbname,*dfile,*dir,*filter,*decfilter,*fullpath;
  Dbptr    db,dba,dbs,dbw,dbz;
  FILE     *fp;
  ...
  if (argc < 16) 
  {
    printf("Usage: %s tstart slen wlen filter sr kmax nk ampfunc beamtype channame arrayname refsta dbname dir dfile\n",argv[0]);
    return 1;
  }
...
/*Now do resampling, if needed.  Find the original sampling rate first.  The
  ...
  if (rdec - floor(rdec) > 0.5) 
    ...
    idec = floor(rdec);
  printf("rdec,idec = %f %d\n",rdec,idec);
  npts = (nptso-1)/idec + 1;  /*new number of points*/
...
/*If series is not a power of two, determine next highest power of two.*/
  if((n=check2(npts))==0)
  {
    n=(int)(log10((double)npts)/log10(2.0))+1;
  }
...
/*Define the slowness grid parameters.*/ 
  ...
  nkx   = nk;
  nky   = nk;
  printf("kxmin,kxmax,kymin,kymax = %f %f %f %f\n",kxmin,kxmax,kymin,kymax);
  printf("nkx,nky = %d %d\n",nkx,nky);
...
/*  do_grid will write the grid, so find byte offset of file now.*/
    ...
    printf("Best ray parameter is: %lf (s/km) \n"
           "Best backazimuth is: %lf (deg) \n"
           "Best x-slowness is: %lf (s/km) \n"
           "Best y-slowness is: %lf (s/km) \n",
...
int do_grid(double del,int index, 
            ...
            double *bkx,double *bky, double *minamp, double *maxamp)
{
	int i,j;
	double val,bestkx,bestky,kx,ky,mina,maxa;
	float *sbeam,*valp;
...
/*      Write entire grid to file as one binary matrix.*/
        fwrite(valp,sizeof(float),(unsigned int)nkx*nky,fout);
