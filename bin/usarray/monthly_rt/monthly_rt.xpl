...
#     $month     = $ARGV[2];
    ...
    foreach $year_month (@year_months) {
#
...
#
            $problems = &rm_db_tables( $problems, qw ( gap netperf chanperf ) ) ;
...
#
            ...
            $problems = &rm_db_tables( $problems, qw ( lastid snetsta schanloc ) ) ;
...
sub rm_tables { $problems = &rm_db_tables( $problems, @tables ) ;
    my ( $problems, @tables ) = @_ ;
    
    foreach $table ( @tables ) {

        $cmd  = "rm $dbname.$table";
 
        if  ( -e "$dbname.$table" && ! $opt_n ) {
            elog_notify( "\n$cmd" ) ;        
            $problems = run( $cmd, $problems ) ;
        } else {
            elog_notify( "\nskipping $cmd" ) ;
