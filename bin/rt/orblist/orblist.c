...

{
  ...
  if ( after > 0 ) {
     if ( ( id = orbafter ( rorb, after-eps )) < 0) {
      ...
      if (orbget (rorb, ORBCURRENT, &id, srcid, &after, 
                              &packet, &nbytes, &bsize)) 
          die(0,"fails to get ORBCURRENT time.\n") ; 
  }


  if( MaxGap == 0) MaxGap =  maxgap; 
...
/* Loop through RB; runnin triggering algorithm  */
  ...
  while(1)  {   
    if( !orbreap( rorb, &id, srcid, &pkttime, &packet, &nbytes, &bsize)  ) {
	 ...
	 if( sort )  {
             switch( orbsort(dtbl->apipe, &id, &pkttime, srcid, &packet, &nbytes, &bsize))  {

	        case 0: 
	          check = 0;	
		  break;
	        case 2:
		   fprintf( stderr, "SRCNAME: %s TIME: %lf -- packet is out of order \n", srcid, pkttime );
	          check = 0;	
		  break;
	        case 3:
		   fprintf( stderr, "SRCNAME: %s TIME: %lf -- duplicate packet \n", srcid, pkttime );
	          check = 0;	
		  break;
	        default:
	          check = 1;	
	 ...
	 if( check )  { 
		   fprintf( stderr, "SRCNAME: %s TIME: %s (%lf) \n", 
		          srcid, s1=strtime(pkttime), pkttime );
			  free(s1);
	           if(dump) hexdump( stderr, packet, nbytes );
	           sval = (short *) packet;
	           hdrsize = *sval;
	           if(hdr) hexdump( stderr, packet+hdrsize, nbytes-hdrsize );
