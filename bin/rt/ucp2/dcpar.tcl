...
#global lcmdwidget -- no last command in a new DC packet
#global lcmdscroll -- no last command in a new DC packet
global DcWin
global numcmd
global cmdarr
global maxcmd
...
# get data parameetr file from a the ucp parametr file 
  if { [catch "pfget $Pfile ParFile" error] } {
      puts "can't find ParFile specifications in a $Pfile parameter file"
      exit 1
  }  else { 
      set dparfile [pfget $Pfile ParFile] 
  }
 
#  get number of DCs in the system from a parameter file
set pfdc [pfgetlist @$Pfile#Dc]

#get srcmatch pattern for DC parameters
  if { [catch "pfget $Pfile SelectDc" error] } {
      puts "can't find SelectDc specifications in a $Pfile parameter file"
      exit 1
  }  else { 
      set dcmatch [pfget $Pfile SelectDc] 
  }
 
#get srcmatch pattern for ALL desired packets 
  if { [catch "pfget $Pfile SelectDas" error] } {
      puts "can't find SelectDas specifications in a $Pfile parameter file"
      exit 1
  }  else { 
      set ddasmatch [pfget $Pfile SelectDas] 
  }
...
set dccom { BATT ACFAIL HAZARD OPEN BUFF SEND } 
set dcspec { CLOCK MCLK ACLK M1 M2 M3 }
...
# NO LAST COMMAND IN a new DC packet 
...
pack $DcWin.control $DcWin.par $DcWin.time \
     -side top -fill both -expand 1 
...
proc get_dcpar { orb }  {
   ...
   global ddasmatch dcmatch dcparval dparfile 
#  global lcmdwidget 
   global dcstarted cmdhistory crnt_cmd      

# set patern for 'orbselect' call -> select only packets with srcname==dcmatch
#   set dcmatch .*BBA/DC

   dcrt dcrtp $orb 
   dcrtp orbsel $dcmatch
   dcrtp select $dcmatch $ddasmatch
