...
int x1, y1, x2, y2;
   {
	...
	case FILL_OR:
		pw_replrop(pw,x1,ymax-y2,dx,dy,PIX_SRC | PIX_DST | PIX_COLOR(pencolor),igl_pr_pat,0,0);
		break;
	case FILL_XOR:
		pw_replrop(pw,x1,ymax-y2,dx,dy,PIX_SRC ^ PIX_DST | PIX_COLOR(pencolor),igl_pr_pat,0,0);
		break;
	case FILL_AND:
		...
		break;
	case FILL_EQU:
		pw_replrop(pw,x1,ymax-y2,dx,dy,PIX_SRC | PIX_COLOR(pencolor),igl_pr_pat,0,0);
		if (fillwhite) {
